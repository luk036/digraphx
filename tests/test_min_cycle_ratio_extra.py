...
def test_set_default_missing_weight() -> None:
    ...
    set_default(digraph, "time", 1)  # type: ignore[arg-type]
...
def test_cycle_ratio_api_zero_cancel() -> None:
    digraph: Dict[str, Dict[str, Dict[str, int]]] = {"a": {"b": {"cost": 5, "time": 1}}}
    api: "CycleRatioAPI[str, Dict[str, int], Fraction]" = CycleRatioAPI(digraph, Fraction)  # type: ignore[arg-type]
    cycle: List[Dict[str, int]] = [digraph["a"]["b"]]
...
def test_min_cycle_ratio_solver_basic() -> None:
    ...
    solver: MinCycleRatioSolver[str, Dict[str, int], Fraction] = MinCycleRatioSolver(digraph)  # type: ignore[arg-type]
...
def test_neg_cycle_finder_howard_no_cycles() -> None:
    ...
    dist: MapAdapter[int] = MapAdapter([0, 0, 0])
    finder = NegCycleFinder(digraph)
    cycles = list(finder.howard(dist, lambda e: e))  # type: ignore[arg-type]
    assert len(cycles) == 0
