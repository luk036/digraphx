...
def test_min_cycle_ratio_with_uniform_cycle(
    n_nodes: int, base_cost: int, base_time: int
) -> None:
...
@given(
    ...
    cycle_costs=st.lists(
        st.integers(min_value=-5, max_value=5), min_size=3, max_size=6
    ),
    cycle_times=st.lists(st.integers(min_value=1, max_value=5), min_size=3, max_size=6),
)
def test_cycle_ratio_with_specific_costs(
    n_nodes: int, cycle_costs: List[int], cycle_times: List[int]
) -> None:
